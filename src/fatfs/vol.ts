...
export function init(volume: { sectorSize: number; numSectors: number; readSectors: (arg0: any, arg1: any, arg2: (e: any) => void) => void; writeSectors: (arg0: any, arg1: any, arg2: any) => void; }, opts: { ro: any; }, bootSector: number[] | null) {
    ...
    if (!BS.BytsPerSec)
        throw Error("This looks like an ExFAT volume! (unsupported)");
    else if (BS.BytsPerSec !== volume.sectorSize)
        throw Error("Sector size mismatch with FAT table.");
    ...
    const firstDataSector =
        BS.ResvdSecCnt + BS.NumFATs * FATSz + rootDirSectors;
    ...
    vol._writeSectors = function (secNum: number, data: string | any[], cb: any) {
        ...
        if (data.length % volume.sectorSize)
            throw Error("Buffer length not a multiple of sector size");
        else if (opts.ro) throw Error("Read-only filesystem");
        else if (secNum < volume.numSectors)
            volume.writeSectors(secNum, data, cb);
        else throw Error("Invalid sector number!");
    };
    ...
    function fatInfoForCluster(n: number) {
        ...
        const FATOffset =
            fatType === "fat12"
                ? Math.floor(n / 2) * entryStruct.size
                : n * entryStruct.size;
